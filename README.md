...
- **Sibling theme share:** chat/`import-design` pulls palette + dual-theme tokens from JamRoast (`jamroast`/`burntjam`) or JamLight (`jamlight`). Self-import (e.g. saying “jampress” on JamPress) is rejected/ignored. When SHARED DESIGN is active it outranks LIVE SITE for palette/tokens/logos; LIVE SITE keeps nav/routes/screen copy. Token excerpts include `.dark`/`.light` ladders, not only `:root`.
- **JamPress recovery (loop `1511ce7f-…`):** rebuild/restart the SlopControl server, then `design_loop_continue` asking to invent new logos **and** adopt theme from JamRoast (or MCP `import-design` with `fromName: "jamroast"`). Prior self-`SHARED_FROM` is ignored automatically; re-import overwrites with a real sibling.
- **Shared design elements (A+B+C):** versioned controls under `.slopcontrol/elements/<id>/vN/` (project library) and optionally `~/.slopcontrol/shared-elements/` (global registry). Each element has `ELEMENT.json`, `SPEC.md`, `mock.html`, optional `tokens.css` + `src/` TS/JS. Author in a central brand project → `design_element_extract` / `design_element_publish` (set `publishToRegistry` for B). Consume in apps via `design_element_import` or chat (“use theme-toggle from jamroast”). Accept freezes `DESIGN_PACK.elements[]`; implement binds `design/elements/` and research must mount by id (prefer `src/` when `hasCode`). Competing day/night toggles while `theme-toggle` is pinned are rejected as drift. Also: `list_design_elements`, `design_element_get`.
...
- **Cross-chat dependency awareness:** ask, agent, plan_loop, and research/draft/develop inject a CROSS-PROJECT DEPS catalog (elements + private npm packages + registered/sibling projects) plus DEPENDENCY INTENT when the operator says things like “use theme-toggle from JamRoast” or “add @jam/…”. MCP/HTTP: `list_cross_project_deps` (`GET /projects/:id/cross-deps`) and `resolve_dependency` (`POST /projects/:id/resolve-dependency`) → then `npm_registry_ensure_rc` → `pnpm add` and/or `design_element_import`. PLAN_PACK may include `dependencies[]` parsed from Likely areas / Handoff notes.
