...
- Change Intent is **LLM-classified** via the `classification` role (`changeKind`, `brandTheming`, `themeWiringOnly`, `requestsMissingThemeControl`, `needsInteraction`), then finalized from those fields only (no description-regex veto on success). Regex heuristics are the sync fallback when the LLM fails or `heuristicOnly` is set. Align / live `tool-*` / overclaim gates stay deterministic and still key off `interaction`.
