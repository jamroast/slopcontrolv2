...
- Change Intent is **LLM-classified** via the `classification` role (`changeKind`, `brandTheming`, `themeWiringOnly`, `requestsMissingThemeControl`, `needsInteraction`), then finalized from those fields only (no description-regex veto on success). Regex heuristics are the sync fallback when the LLM fails or `heuristicOnly` is set. PHASE↔Intent **alignment** is now **LLM-judged** too (`judgeIntentAlignmentViaLlm` / `phaseDocAlignsWithChangeIntentAsync`): the deterministic keyword gate flags candidate gaps, the judge arbitrates semantics (e.g. a bubble-mount `FormBubble` / `sendFormAnswer` proof counts even though the regex only knows `composer-form`), and a judge error keeps the deterministic issue (fail closed). Live `tool-*` gates stay deterministic and still key off `interaction`.
...
- RESEARCH that overclaims “~90% already works” without residual risks is retried once when `interaction` is set (skipped for `chrome-hide` / `backend`). This overclaim check is **LLM-judged** too (`judgeResearchEngagementViaLlm` / `researchEngagementQualityAsync`): the deterministic overclaim regex flags candidates, the judge arbitrates whether a residual risk is documented in a form the keyword list missed, and a judge error keeps the deterministic issue (fail closed).
