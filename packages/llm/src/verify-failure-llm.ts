...
Classes:
- "infra": external service/infrastructure failure — database, redis, or dev-server connection refused; "port already allocated/in use"; Docker daemon/network errors; outbound fetch/connect failures ("fetch failed", "connect timeout", getaddrinfo/ENOTFOUND); model-provider entitlement or quota rejections (HTTP 401/403/429 from the LLM provider).
