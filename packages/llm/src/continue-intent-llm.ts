...
- shareFrom: optional string — when adoptTheme or adoptChrome is true, the sibling PROJECT name, folder basename, or absolute path. Normalize npm package names to project names: @scope/name → scope-name (e.g. @jamroast/components → jamroast-components). Omit when not adopting from a sibling.
...
- When the operator names an npm package like @scope/name (e.g. @jamroast/components), set shareFrom to the PROJECT name scope-name (e.g. jamroast-components), NOT the package string. Strip the leading @ and replace / with -.
