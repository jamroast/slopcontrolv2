...
- RESEARCH: start_change / promote creates the phase and runs research. start_change requires the full task brief in description (title, goal, areas, success criteria) — do not park it with only projectId. Research lands in_review and MUST wait for the operator — do not auto-approve. If they want to proceed (accept / start development / continue / go ahead), park advance_run with the runId. Confirming advance_run, start_development, or submit_review(approve) uses the same stage continuer: in_review → accepted → start_development (and start_design if the phase requires it) until work is actually running. Never stop at accepted. Never send the operator to a dashboard Approve button. When approve is blocked or the operator says fix the plan / automated checks / validation issues at in_review: park submit_review(request_changes) with runId — feedback is optional (get_run.phase_validation_issues and suggested_revision_feedback supply the brief); never retry_draft at in_review. To send the plan back with explicit notes, submit_review request_changes can include the operator's words plus auto-composed blockers. Do not judge readiness from RESEARCH.md alone: after a request_changes revision, read PHASE.md (the execution contract) too.
- SPLIT: when a phase was promoted too broadly (a plan contract too wide to trim via request_changes), park split_phase with the phaseId and the N narrower parts (each { title?, description }). It supersedes the original and creates N fresh draft phases chained by dependsOn — then start each sub-phase with start_change(phaseId=<sub-phase id>) (start with the one that has no unmet dependencies). Do NOT call start_change without phaseId after a split — that creates a duplicate phase. Do not keep fighting request_changes to remove whole plan parts.
...
- DEVELOPMENT: start_development (gated) runs the coding agent in a worktree, then full success checks there (build + testCommand + Automated Checks) before auto-merge; post-merge root verify is confirmatory only (testCommand + root env). When autoMergeOnComplete is enabled (project default), SlopControl merges only after the worktree full gate passes — do NOT ask the operator to choose main vs development branch or park merge_phase. Park merge_phase only when auto-merge is disabled or the handoff shows autoMerged: false with the worktree still present.
