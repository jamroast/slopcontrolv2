...
const CHAT_TOOL_DESCRIPTION: Record<string, string> = {
  ...
  design_element_extract:
    "Extract a shared element from a design-loop mock and publish to the project library. Prefer list_extractable_design_elements first, then pass the listed elementId (and optional label). Without elementId, defaults to theme-toggle when present. Optional publishToRegistry. Pass loopId explicitly — do NOT rely on this chat's design-loop latch (it may point at a different project's loop). Works on any loop status (open/accepted/implemented) — no need to re-accept first. When the operator names a specific version (e.g. 'extract v9' / 'from the accepted v9'), pass that version — do NOT default to the current tip. Extracting an EXISTING elementId bumps it to the next version (evolve) — use this when a design_loop_accept result surfaces an `evolve` directive for a pinned element.",
  design_element_import:
