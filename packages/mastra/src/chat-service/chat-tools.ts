...
const CHAT_TOOL_DESCRIPTION: Record<string, string> = {
  ...
  design_loop_start:
    "Start a look-and-feel design loop (mock HTML exploration). Requires brief — pass the operator's words in brief. In global chat always pass projectId. When several loops exist on a project, pass loopId on accept/discard/continue/get. Notification-driven when live turn active. If the project already has a finalized (implemented/accepted) design loop for the same surface, prefer design_loop_continue on that loop instead of starting a fresh one.",
  design_loop_accept:
  ...
  implement_design:
    "Bind the accepted design loop to a phase (writes UI-SPEC, tokens.css, design/mock.html, DESIGN_COMPLETE; starts research). Call this AFTER design_loop_accept. Pass loopId, or omit to use this chat's latched (just-accepted) loop; global chat must pass projectId. If the accept result surfaced an `evolve` directive: when it says COMPOSE <sub> inside <pinned>, mount <sub> inside the pinned shell (extract <sub> first only if it is not already in a library); when it says EVOLVE, run design_element_extract + design_library_publish (and pnpm add the new version in the consumer) BEFORE implement_design — do not implement the stale pinned version.",
  relaunch_design_research:
