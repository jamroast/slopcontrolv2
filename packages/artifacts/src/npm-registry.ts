...
/** Built-in default — project config / .npmrc discovery widens this. */
export const NPM_PRIVATE_SCOPES = ["@slopcontrol"] as const;

/**
 * Resolve the private npm scopes for a project.
 * Order: project config registryScopes → scopes discovered from the
 * project's .npmrc scoped-registry lines → ["@slopcontrol"].
 * Jam projects keep working unchanged: their .npmrc already carries the
 * estate scopes, or config pins them explicitly.
 */
export function resolveProjectRegistryScopes(opts: {
  projectRoot: string;
  configScopes?: string[];
}): string[] {
  ...
  if (existsSync(npmrcPath)) {
    try {
      ...
      for (const line of readFileSync(npmrcPath, "utf-8").split("\n")) {
        const m = line.match(/^\s*(@[\w.-]+):registry=/i);
        if (m?.[1]) scopes.add(m[1]);
      }
      if (scopes.size > 0) return [...scopes].sort();
    } catch {
      /* fall through */
    }
  }
...
export function writeNpmRegistryMeta(
  ...
  // Union in scopes from published packages so consumers' .npmrc covers
  // whatever this registry has ever served.
  const publishedScopes = Object.keys(meta.publishedPackages ?? {})
    .map((name) => name.match(/^(@[\w.-]+)\//)?.[1])
    .filter((s): s is string => Boolean(s));
  meta.scopes = [
    ...new Set([...meta.scopes, ...publishedScopes]),
  ].sort();
...
export function buildVerdaccioConfigYaml(opts: {
  ...
  host: string;
  port: number;
  /** Private scopes published locally (default: @slopcontrol). */
  scopes?: string[];
}): string {
...
export function ensureNpmRegistryLayout(dataDir: string): NpmRegistryMeta {
  ...
  // Scopes = default + scopes already published + prior configured scopes
  // (existing registries keep their estate scopes; new projects add theirs
  // on publish via the same union in recordPublishEvidence).
  const priorScopes = prior?.scopes ?? [];
  const publishedScopes = Object.keys(prior?.publishedPackages ?? {})
    .map((name) => name.match(/^(@[\w.-]+)\//)?.[1])
    .filter((s): s is string => Boolean(s));
  const scopes = [
    ...new Set([...NPM_PRIVATE_SCOPES, ...priorScopes, ...publishedScopes]),
  ].sort();
...
export function jamPackageNameForElement(
  elementId: string,
  scope: string = NPM_PRIVATE_SCOPES[0],
): string {
  const id = elementId
    .trim()
    .toLowerCase()
...
export function scaffoldElementNpmPackage(opts: {
  ...
  mockHtml?: string;
  tokensCss?: string;
  /** Package scope (default: first resolved registry scope). */
  scope?: string;
}): {
  packageRoot: string;
  packageName: string;
  ...
  const packageName = jamPackageNameForElement(opts.elementId, opts.scope);
